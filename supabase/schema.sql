...
begin
  ...
  insert into public.products (org_id, name, name_en, description, description_en, price, emoji)
  values (v_org, trim(p_name), nullif(trim(p_name_en), ''), nullif(trim(p_description), ''),
    nullif(trim(p_description_en), ''), p_price, nullif(trim(p_emoji), '')) returning id into v_product;
  return v_product;
end $$;
revoke all on function public.create_ngo_product(text, text, text, text, numeric, text) from public, anon, authenticated;
grant execute on function public.create_ngo_product(text, text, text, text, numeric, text) to authenticated;
