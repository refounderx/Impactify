...
**Campaign editing ready in code (2026-08-28):** the NGO campaigns screen opens the existing six-step wizard with the selected campaign prefilled, and `20260828120000_update_campaign.sql` adds a tenant-derived atomic update RPC. The hosted migration is not yet applied: the linked Supabase CLI could not obtain its temporary database login role (`LegacyDbConfigLoginRoleNetworkError`). Apply the migration before treating edit-save as live in the hosted environment.
