...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    void Promise.all([getProductsByIds([id]), campaignId ? getCampaignById(campaignId) : Promise.resolve(null)]).then(async ([products, currentCampaign]) => {
    ...
    <div className="mx-auto max-w-6xl px-5 py-8 md:px-8">
      ...
      <div className="mt-6 grid gap-8 rounded-[2rem] bg-white p-6 shadow-sm md:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)] md:p-10">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-3 text-center text-sm font-bold text-slate-500"><Heart className="me-1 inline text-pink-500" size={16} fill="currentColor" />{(campaign?.donors ?? 0).toLocaleString()} {lang === "en" ? "people have already donated" : "כבר בחרו לתרום"}</p>
          {campaign && <div className="mt-6 flex flex-wrap gap-3 border-t border-slate-100 pt-5"><Link href={`/organization/${campaign.orgId}`} className="rounded-full border border-raz-teal px-4 py-2 text-sm font-bold text-raz-teal">{lang === "en" ? "About the nonprofit" : "לעמוד העמותה"}</Link><Link href={`/campaign/${campaign.id}`} className="rounded-full border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600">{lang === "en" ? "Campaign page" : "לעמוד הקמפיין"}</Link></div>}
        </div>
      </div>
