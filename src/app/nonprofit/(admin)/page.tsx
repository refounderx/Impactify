...
export default function NgoDashboardPage() {
  ...
  const [backlog, setBacklog] = useState<CampaignJoinRequest[]>([]);
  const [requestBusy, setRequestBusy] = useState<string | null>(null);
  async function loadRequests() { const [inbox, queued] = await Promise.all([getNgoCampaignRequests(), getPartnershipRequests("backlog")]); setRequests(inbox); setBacklog(queued); }
  useEffect(() => {
    const timer = window.setTimeout(() => { void loadRequests().catch(() => { setRequests([]); setBacklog([]); }); }, 0);
    return () => window.clearTimeout(timer);
  }, []);
  async function decideRequest(request: CampaignJoinRequest, action: "approve" | "reject") { setRequestBusy(request.id); try { await manageNgoCampaignRequest(request.id, action); await loadRequests(); reload(); } finally { setRequestBusy(null); } }
  ...
  return <div>
    ...
    <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
      {requests.length > 0 && <div className="border-b border-slate-200 bg-teal-50/60 px-8 py-5"><h2 className="font-bold text-slate-900">{lang === "en" ? "Partnership requests" : "בקשות שותפות"}</h2><div className="mt-3 space-y-2">{requests.map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-3 text-sm"><span>{request.community_name} · {request.campaign_title}</span><span className="flex gap-2"><button disabled={requestBusy === request.id} onClick={() => void decideRequest(request, "reject")} className="rounded-lg px-3 py-1.5 text-red-600 hover:bg-red-50">{lang === "en" ? "Reject" : "דחייה"}</button><button disabled={requestBusy === request.id} onClick={() => void decideRequest(request, "approve")} className="rounded-lg bg-raz-teal px-3 py-1.5 font-bold text-white">{lang === "en" ? "Approve" : "אישור"}</button></span></div>)}</div></div>}
      {backlog.length > 0 && <div className="border-b border-slate-200 bg-slate-50 px-8 py-4"><p className="text-sm font-bold text-slate-700">{lang === "en" ? `${backlog.length} requests are waiting in the outreach backlog` : `${backlog.length} בקשות ממתינות בתור`}</p><div className="mt-3 flex flex-wrap gap-2">{backlog.map((request) => <button key={request.id} type="button" disabled={requestBusy === request.id} onClick={() => void decideRequest(request, "approve")} className="rounded-full border border-raz-teal bg-white px-3 py-1.5 text-xs font-bold text-raz-teal">{request.community_name} · {lang === "en" ? "Approve now" : "אישור מיידי"}</button>)}</div></div>}
