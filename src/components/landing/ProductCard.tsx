...
export default function ProductCard({
  ...
  return (
    <div role={onOpenDetails ? "button" : undefined} tabIndex={onOpenDetails ? 0 : undefined} onClick={onOpenDetails} onKeyDown={(event) => { if (onOpenDetails && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onOpenDetails(); } }} className={`flex min-h-[19rem] flex-col rounded-[1.5rem] bg-white p-5 text-center shadow-[0_8px_22px_rgba(15,23,42,0.1)] ring-1 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_15px_30px_rgba(15,23,42,0.14)] ${onOpenDetails ? "cursor-pointer" : ""} ${isChosen ? "ring-2 ring-raz-teal" : "ring-slate-100"}`}>
      {(imageUrl || videoUrl) && <div aria-label={imageUrl ? undefined : emoji} className={`relative mb-4 flex overflow-hidden rounded-2xl ${imageUrl ? "h-40 bg-slate-50" : "h-32 bg-raz-dark"}`}>
        ...
        {videoUrl && <button type="button" onClick={(event) => { event.stopPropagation(); onOpenDetails?.(); }} className="absolute inset-0 flex items-center justify-center bg-raz-dark/20 transition hover:bg-raz-dark/35" aria-label="צפייה בפרטי המוצר"><span className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-white bg-raz-dark/65 text-white shadow-lg"><Play size={22} fill="currentColor" /></span></button>}
      </div>}
      ...
      <div className="mt-auto">{campaignCta ? (
        <button type="button" onClick={(event) => { event.stopPropagation(); onChoose?.(); }} className="interactive-control w-full bg-raz-teal text-white font-bold py-2 rounded-full text-sm mb-2">
