    ...
    <section id="contact" className="max-w-6xl mx-auto px-6 py-10">
      <div className="bg-white border border-gray-100 rounded-2xl px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button type="button" onClick={() => router.push("/search")} className="interactive-control bg-raz-teal text-white text-sm font-bold px-4 py-2 rounded-full"><EditableText tKey="landing.cta.donateBtn" /></button>
      ...
      <div className="bg-raz-teal rounded-2xl px-6 py-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3">
          <Link href="/auth" className="interactive-control bg-white text-raz-teal font-bold px-5 py-2.5 rounded-full text-sm"><EditableText tKey="landing.cta.noOrg" /></Link>
          <Link href="/auth" className="interactive-control bg-raz-dark text-white font-bold px-5 py-2.5 rounded-full text-sm"><EditableText tKey="landing.cta.hasOrg" /></Link>
        </div>
      </div>
    </section>
  );
