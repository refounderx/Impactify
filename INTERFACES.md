...
| `supabase/migrations/20260904120000_fix_partnership_requests_org_id_ambiguity.sql` | Qualifies profile columns in the partnership-inbox RPC, resolving the `org_id` ambiguity | Apply through Supabase SQL Editor to restore partnership inbox loading |
| `supabase/migrations/20260908120000_product_first_donations.sql` | Makes `donations.campaign_id` nullable, adds per-campaign `required_quantity`, and exposes aggregate product discovery/progress RPCs | Pending: `supabase db push` could not authenticate the linked database during this change |
...
| `supabase/migrations/20260908152000_restore_discoverable_product_media.sql` | Restores `image_url` and `video_url` to standalone-product discovery RPCs used by public product cards | Apply through Supabase SQL Editor after `20260908120000_product_first_donations.sql` |
